...
do
    ...
    OUTPUT=$(python3 weak_prng.py -n $NUM_STRINGS 2>&1 | grep "Chi-squared value")
    
    # Extract the chi-squared value from the output
    CHI_SQUARED=$(echo $OUTPUT | awk '{print $3}')
    
    # Add the chi-squared value to the total
    TOTAL_CHI_SQUARED=$(echo "$TOTAL_CHI_SQUARED $CHI_SQUARED" | awk '{print $1 + $2}')
done

# Calculate the average chi-squared value
AVERAGE_CHI_SQUARED=$(echo "$TOTAL_CHI_SQUARED $NUM_RUNS" | awk '{print $1 / $2}')
